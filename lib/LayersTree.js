...
LayersTree.prototype.getStateSince = function (timestamp) {
  return {
    tombstones: this._tombstoneSet.getStateSince(timestamp),
    data: this._dataMap.getStateSince(timestamp),
    positions: this._positionMap.getStateSince(timestamp)
  };
};
...
LayersTree.prototype._breakCycles = function () {
  var positionData = this._positionMap.getValue();
  var ids = Object.keys(positionData).sort();
  ...
  for (var i = 0; i < ids.length; i++) {
    ...
    checkedIds[id] = true;
    var currId = id;
    var pos = positionData[id];

    while (pos.parent !== null && checkedIds[pos.parent] !== true) {
      currId = pos.parent;
      checkedIds[currId] = true;
      pos = positionData[currId];
    }

    if (pos.parent !== null) {
      // THERE IS A CYCLE, MOVE THE TOP ELEMENT
      this.move(currId, null);
      positionData = this._positionMap.getValue();
    }
  }
};
