...
LayersTree.prototype._insertValue = function (dataMap, positionMap, valueMap, id) {
  if (valueMap[id]) {
    return valueMap[id];
  }
  ...
  if (!!pos.parent) {
    this._insertValue(dataMap, positionMap, valueMap, pos.parent).children.push(id);
  }
...
LayersTree.prototype.getValue = function () {
  if (typeof this._value === 'undefined') {
    ...
    // find the full list of ids to show including un-tombstoned ids

    // untombstone anything that needs to be un-tombstoned

    // construct the full tree / sorted list

    for (var id in dataMap) {
      if (tombstones.indexOf(id) === -1) {
        this._insertValue(dataMap, positionMap, tempMap, id);
      }
    }
...
LayersTree.prototype.move = function (id, parentId, startEl, endEl) {
  ...
  console.log('INSERTING', id, 'AT', pos, 'BETWEEN', startPos, 'AND', endPos);
